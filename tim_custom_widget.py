...
class ResultDialog(QDialog):
	...
	def __init__(self, parent):
		...
		self.dic_widget = {
			'processed': {'label': ClickableLabel('Processed'), 'txt': QTextEdit(), 'btn_copy': QPushButton('Copy')},
			'plaintext': {'label': ClickableLabel('Plaintext ▲▼'), 'txt': QTextEdit(), 'btn_copy': QPushButton('Copy')},			
			'original': {'label': ClickableLabel('Original ▲▼'), 'txt': QTextEdit(), 'btn_copy': QPushButton('Copy')}
		}
	...
	def init(self):
		...
		self.resize(600, 600)
		label_style = 'background: #ffffff; font-family: verdana; font-size: 16px; color: #333333;'
		txt_style = '''color: #E4E4E4;
			border-radius: 8px; border: 1px solid #CBC9D2; padding: 10px;
			'''
		btn_style = '''background: qlineargradient( x1:0 y0:1, x2:1 y2:0, stop:0 #242F49, stop:1 #6281C8); color: #E4E4E4;
			border-width: 2px; border-radius: 12px;
			padding: 5px;
			width: 70px;
			font-size: 13px;'''
		form_style = '''border-width: 2px; border-radius: 8px;
			padding: 6px;
			width: 140px;			
			font-size: 16px;
			background-color: #d2d6dd; color: #159ddd;'''
		...
		[widget.setStyleSheet(btn_style) for widget in [self.dic_widget[x]['btn_copy'] for x in self.dic_widget]]
		...
		self.dic_widget[label]['btn_copy']])) for label in self.dic_widget if label != 'processed']
		self.show()
